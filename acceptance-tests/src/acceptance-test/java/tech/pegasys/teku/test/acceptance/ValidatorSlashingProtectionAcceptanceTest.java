...
public class ValidatorSlashingProtectionAcceptanceTest extends AcceptanceTestBase {
  ...
  void shouldShutDownVC() throws Exception {
    ...
    final TekuNode secondBeaconNode =
        createTekuNode(
            config ->
                configureNode(config, genesisTime)
                    .withAltairEpoch(altairEpoch)
                    .withValidatorSlashingProtectionEnabled()
    ...
    secondBeaconNode.waitForLogMessageContaining(
        String.format(
            "Validator(s) with public key(s) %s got slashed",
